    ...
    <div class="container-xxl d-flex flex-stack flex-wrap">
        <div class="page-title d-flex flex-column me-3">
            ...
            <ul class="breadcrumb breadcrumb-dot fw-bold text-gray-600 fs-7 my-1">
                <li class="breadcrumb-item text-gray-600"><a href="{{ url('/shop/dashboard') }}" class="text-gray-600 text-hover-primary">Dashboard</a></li>
        ...
        <div class="row g-5">
                                ...
                                @forelse($categories as $category)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <form method="POST" action="{{ route('shop.categories.destroy', $category->id) }}" class="d-inline" onsubmit="return confirm('Delete this category?');">
                                                @csrf
                                                @method('DELETE')
                                    ...
                                    @foreach($category->children as $child)
                                        <tr>
                                            ...
                                            <td class="text-end">
                                                ...
                                                <form method="POST" action="{{ route('shop.categories.destroy', $child->id) }}" class="d-inline" onsubmit="return confirm('Delete this category?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-sm btn-light-danger">Delete</button>
                                                </form>
            ...
            <div class="col-xl-5">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('shop.categories.store') }}" id="category-form">
                            @csrf
...
<script>
    function editCategory(id, name, parentId, icon, description, isActive) {
        ...
        document.getElementById('category-form').action = '{{ url("/shop/categories") }}/' + id;
    ...
    function resetForm() {
        ...
        document.getElementById('category-form').action = '{{ route("shop.categories.store") }}';
