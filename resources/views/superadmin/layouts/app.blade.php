...
<body>
    ...
    <aside class="app-sidebar" id="appSidebar">
        ...
        <nav class="nav flex-column flex-grow-1">
            ...
            <a href="{{ route('superadmin.shops') }}" class="nav-link {{ request()->routeIs('superadmin.shops') ? 'active' : '' }}">
                <i class="bi bi-shop"></i> Shops
            </a>
            <a href="{{ route('superadmin.admins') }}" class="nav-link {{ request()->routeIs('superadmin.admins') ? 'active' : '' }}">
                <i class="bi bi-person-badge-fill"></i> Admins
            </a>
            <a href="{{ route('superadmin.invoices.index') }}" class="nav-link {{ request()->routeIs('superadmin.invoice*') ? 'active' : '' }}">
                <i class="bi bi-receipt"></i> Invoices
            </a>
            <a href="{{ route('superadmin.reports.index') }}" class="nav-link {{ request()->routeIs('superadmin.reports.index') ? 'active' : '' }}">
                <i class="bi bi-bar-chart-fill"></i> Reports
            </a>

            <div class="nav-section-title">Catalog</div>
            <a href="{{ route('superadmin.brands.index') }}" class="nav-link {{ request()->routeIs('superadmin.brands.index') ? 'active' : '' }}">
                <i class="bi bi-tags-fill"></i> Brands
            </a>
            <a href="{{ route('superadmin.categories.index') }}" class="nav-link {{ request()->routeIs('superadmin.categories.index') ? 'active' : '' }}">
                <i class="bi bi-collection-fill"></i> Categories
            </a>

            <div class="nav-section-title">Access Control</div>
            <a href="{{ route('superadmin.roles.index') }}" class="nav-link {{ request()->routeIs('superadmin.roles.index') ? 'active' : '' }}">
                <i class="bi bi-people-fill"></i> Roles
            </a>
            <a href="{{ route('superadmin.permissions.index') }}" class="nav-link {{ request()->routeIs('superadmin.permissions.index') ? 'active' : '' }}">
                <i class="bi bi-key-fill"></i> Permissions
            </a>
        </nav>

        <a href="{{ route('superadmin.logout') }}" class="nav-link mt-auto">
        ...
        <header class="app-topbar">
            ...
            <div class="search-box">
                <div class="input-group">
                    ...
                    <input type="search" id="globalSearchInput" class="form-control ps-5" placeholder="Search shops, admins, invoices...">
                </div>
            </div>
            ...
            <div class="ms-auto d-flex align-items-center gap-3">
                ...
                <div class="dropdown">
                    <a class="d-flex align-items-center gap-2 dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">
                        <img src="https://ui-avatars.com/api/?background=22c55e&color=fff&name={{ urlencode(auth('superadmin')->user()->name ?? 'Super Admin') }}" class="avatar-sm">
                        <span class="fw-semibold">{{ auth('superadmin')->user()->name ?? 'Super Admin' }}</span>
                    </a>
                    <ul class="dropdown-menu dropdown-menu-end">
                        <li><a class="dropdown-item" href="{{ route('superadmin.logout') }}"><i class="bi bi-box-arrow-right me-2"></i>Logout</a></li>
                    </ul>
                </div>
            </div>
        </header>
    ...
    <script>
        ...
        // Auto-fill search box from ?q= and keep focus/value on typing (simple client-side autofill of last search term)
        (function () {
